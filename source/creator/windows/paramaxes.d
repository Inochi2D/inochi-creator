...
protected:
    ...
    void onBeginUpdate() {
        ...
        igSetNextWindowSizeConstraints(ImVec2(384, 384), ImVec2(float.max, float.max));
        super.onBeginUpdate();
    }

    void axisPointList(ulong axis) {
        int deleteIndex = -1;
    ...
    void onUpdate() {
        igPushID(cast(void*)param);
            ImVec2 avail = incAvailableSpace();
            float reqSpace = param.isVec2 ? avail.y - 24 : 32;

            if (igBeginChild("###ControllerView", ImVec2(avail.x/2, reqSpace))) {
                incControllerAxisDemo("###CONTROLLER", param, points, ImVec2(avail.x/2, reqSpace));
                igEndChild();
            }
